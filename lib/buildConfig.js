...
/**
 * Build self.config, call `cb` on completion.
 * 
 ...
 * `doc_base`: basename of docs output. 
...
module.exports = function buildConfig (cb) {
  ...
  //use default template and css (user options can overwrite this)
  ...
  //config keys that are relative to writeme root
  var relativePaths = { template_path: 1, css_path: 1 }
  ...
  if (fs.existsSync(userConfigPath)) {
    ...
    for (var override in userConfig) {
      var userValue = userConfig[override]

      self.config[override] = relativePaths[override] 
                            ? path.resolve(self.root, userValue)
                            : userValue
    }
